...
- `ai.model=${AI_MODEL:google/gemma-4-26b-a4b-it:free}` — el tier gratuito de OpenRouter son **20 req/min y 50 req/día** (1.000/día si alguna vez se compran $10 en créditos). Al agotarse entra el mensaje de respaldo. **Era `gemma-4-31b-it:free`**, que estuvo devolviendo 502 durante una hora larga (fallo del proveedor: el mismo modelo fallaba igual en local y en producción, mientras este otro respondía). Ese modelo ya se recuperó, así que los dos sirven; este es más pequeño y rápido. Lo importante del episodio: **cambiar de modelo no necesita despliegue** — es `AI_MODEL` en Railway, y el valor de este fichero solo se aplica si esa variable no está puesta. Conviene comprobar cuál de las dos manda antes de tocar el código.
