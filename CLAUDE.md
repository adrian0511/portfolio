...
- **`ChatExceptionHandler`** (`@RestControllerAdvice`): traduce `AiClientException` a texto útil con **200**, no a un error. Distingue 429 (límite), 402 (sin crédito) y el resto. **Registra el modelo en la línea de error, entre comillas**, y `ChatService` lo deja también al arrancar: un `400` significa que OpenRouter rechaza el id, así que sin saber qué id le llegó no se puede diagnosticar. Pasó en producción —`AI_MODEL` mal puesta en Railway— y el log solo decía `statusCode=400`; las comillas hacen visibles los espacios o comillas que viajen pegados al valor. **Limitación real**: solo captura fallos *previos al primer token* (sin API key, 401, 429, red), que son los habituales porque `stream(...)` falla en la petición inicial. Un fallo a mitad de stream (`STREAM_ERROR`) llega con la respuesta ya comprometida y el visitante vería la respuesta truncada.
...
- `ai.model=${AI_MODEL:google/gemma-4-31b-it:free}` — el tier gratuito de OpenRouter son **20 req/min y 50 req/día** (1.000/día si alguna vez se compran $10 en créditos). Al agotarse entra el mensaje de respaldo. **Si el modelo se cae, el chat entero se cae con él**: este estuvo devolviendo 502 durante una hora larga (fallo del proveedor — fallaba igual en local y en producción, mientras `gemma-4-26b-a4b-it:free` respondía bien) y todo visitante veía el mensaje de respaldo. Se recuperó solo. La salida rápida no es tocar código: **cambiar `AI_MODEL` en Railway no necesita despliegue**, y el valor de este fichero solo se aplica si esa variable no está puesta, así que conviene mirar cuál de las dos manda antes de editar nada.
