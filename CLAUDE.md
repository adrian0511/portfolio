...
- `ChatRateLimitFilterTest` — cupo por sesión: dentro pasa, al superarlo 429 sin llamar al modelo, sesiones distintas no comparten cupo, otras rutas no consumen. Y el cupo por IP: en IPv6 va por **/64** (rotar la dirección dentro del prefijo no estrena cupo) sin que dos prefijos distintos se pisen, una IPv4 mapeada (`::ffff:…`) sigue contando como IPv4, y un valor que no es una IP literal se agrupa tal cual en vez de romper el cupo.
